...
function ExperienceListItem({ item }: { item: ExperienceItem }) {
    ...
    return (
        <li className="flex items-start gap-3">
            ...
            <div className="w-3 h-3 border-2 border-violet-500 dark:border-violet-400 mt-1.5 flex-shrink-0 rounded-sm" />
...
export function EducationCard({ data, isEditMode, onEducationChange }: EducationCardProps) {
  return (
    <Card className="shadow-sm border-border bg-card dark:bg-gray-800 dark:border-gray-700">
      <CardHeader className="pb-6">
        <div className="flex items-center gap-2">
          <CardTitle className="text-lg text-foreground dark:text-gray-100">Education & Certifications</CardTitle>
        </div>
      </CardHeader>

        <CardContent>
            {isEditMode ? (
                <ExperienceForm items={data} onChange={onEducationChange} showCertificates={true} />
            ) : (
