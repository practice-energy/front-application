...
export const SearchBar = React.memo(function SearchBar({
  ...
  const dynamicStyles = useMemo(() => {
    ...
    const currentSidebarWidth = sidebarCollapsed ? 0 : sidebarWidth || 378
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            {uploadedFiles.length > 0 && (
              <div className="mb-2 flex flex-wrap gap-2">
                {uploadedFiles.map((file, index) => (
                  <div
                    key={index}
                    className="flex items-center gap-2 bg-gray-100 dark:bg-gray-700 rounded-lg px-2 py-1 text-xs"
                  >
            ...
            <div
              className={`relative backdrop-blur-sm p-3 transition-all duration-300 flex items-center gap-2 rounded-lg border
                ${
                  isDragOver
