...
export function ShareServiceModal({ isOpen, onClose, service }: ShareServiceModalProps) {
  const [copied, setCopied] = useState(false)
  const [customText, setCustomText] = useState("")

  const shareUrl = `${window.location.origin}/service/${service.id}`
  const defaultText = `Check out this service: ${service.title}`
  ...
  const handleShare = (platform: string) => {
    ...
    const shareUrls = {
      ...
      email: `mailto:?subject=${encodeURIComponent(`Check out ${service.title}`)}&body=${encodedText}%0A%0A${encodedUrl}`,
    }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700">
        <DialogHeader>
          <DialogTitle className="text-gray-900 dark:text-gray-100">Share Service</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-300">{service.specialist.name}</p>
            </div>
          </div>
