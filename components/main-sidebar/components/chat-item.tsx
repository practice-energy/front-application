        ...
        <div className={cn("flex-1 min-w-0", isCollapsed && !isMobile ? "hidden" : "block")}>
          ...
          <div className="flex items-start justify-between mb-1">
            ...
            <div className="flex items-center gap-2 ml-2">
              ...
              {hasUnread && <div className="w-4 h-4 bg-violet-500 rounded-sm flex-shrink-0" />}

              {/* Close button */}
              <button className="w-4 h-4 flex items-center justify-center text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
          ...
          {/* Message preview */}
          <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">{chat.description}</p>
        </div>
      </div>
    </div>
  )
