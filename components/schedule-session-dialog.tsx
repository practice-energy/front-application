...
interface Specialist {
  id: string
  name: string
  services: Array<{ name: string; duration: string; price: number }>
}
...
export function ScheduleSessionDialog({
  ...
  useEffect(() => {
    if (isReschedule && client?.service && !bookingForm.service) {
      setBookingForm((prev) => ({
        ...prev,
  ...
  }, [isReschedule, client?.service, bookingForm.service])

  useEffect(() => {
    // Simulate fetching existing bookings for the selected date
    if (bookingForm.date) {
      // In a real application, you would fetch this data from your backend
      const bookings = mockExistingBookings.filter((booking) => booking.date === bookingForm.date)
  ...
  useEffect(() => {
    if (bookingForm.date && bookingForm.time) {
      const validation = validateTimeSlot(bookingForm.date, bookingForm.time)
      setTimeValidation(validation)
    } else {
      setTimeValidation(null)
    }
  }, [bookingForm.date, bookingForm.time, existingBookings])

  const validateTimeSlot = (date: string, time: string) => {
    const startTime = new Date(`${date}T${time}:00`)
    const endTime = new Date(startTime.getTime() + Number.parseInt(bookingForm.duration) * 60000)

    for (const booking of existingBookings) {
      const bookingStartTime = new Date(`${booking.date}T${booking.time}:00`)
      ...
      if (startTime < bookingEndTime && endTime > bookingStartTime) {
        return { available: false, reason: "Time slot overlaps with an existing booking." }
      }
    }
  ...
  const handleDateSelect = (date: string) => {
    setBookingForm((prev) => ({ ...prev, date: date, time: "" })) // Reset time when date changes
    setTimeValidation(null) // Reset time validation when date changes
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{isReschedule ? "Reschedule Session" : "Schedule Session"}</DialogTitle>
          <DialogDescription>
            {isReschedule
              ? "Adjust the date or time for your session."
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="space-y-3">
            <Label className="text-lg font-semibold text-foreground">Service Type</Label>
            {isReschedule ? (
              <Card className="p-4 bg-gray-50 border-gray-200">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Service type cannot be changed when rescheduling</p>
                  </div>
                </div>
              </Card>
            ) : (
          ...
          {/* Date Selection */}
          <div className="space-y-3">
            <Label className="text-lg font-semibold text-foreground">Choose a Date</Label>
            <Card className="p-4">
          ...
          {bookingForm.date && (
            <div className="space-y-3">
              <Label className="text-lg font-semibold text-foreground">Available Times</Label>
              <TimeAvailabilityDiagram
                selectedDate={bookingForm.date}
                onTimeSelect={handleTimeSelect}
                existingBookings={existingBookings}
              />
              {bookingForm.time && timeValidation && (
                <div
                  className={`p-3 rounded-md flex items-center gap-1.5 ${
                    timeValidation?.available ? "bg-green-500 text-green-50" : "bg-red-500 text-red-50"
                  }`}
                >
                  {timeValidation?.available ? (
                    ...
                    <>
                      ...
                      <span>{timeValidation.reason}</span>
                    </>
                  )}
                </div>
              )}
