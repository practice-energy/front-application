...
```
.github/workflows/
├── ci.yml                   # Основной CI процесс (lint, test, build)
├── deploy-cloudflare.yml    # Полный деплой с проверками
├── quick-deploy.yml         # Быстрый деплой без проверок
└── quality-check.yml        # Проверка качества кода
```
...
### ✅ Исправлена проблема с pnpm

Все workflow файлы содержат правильную настройку кэширования pnpm и должны работать корректно.
...
1. **Push в main/master/prod** → Запуск CI → Деплой в production
2. **Push в dev** → Запуск CI → Деплой в staging
3. **Pull Request** → Запуск CI → Деплой preview версии

#### Быстрый деплой (без проверок):
1. **Push в main/master/prod** → Быстрый деплой в production
2. **Push в dev** → Быстрый деплой в staging
3. **Ручной запуск** → Выбор окружения для деплоя

### Пропуск проверок

Для пропуска проверок добавьте в сообщение коммита:
- `[skip lint]` - пропустить проверку линтера
- `[skip test]` - пропустить тесты
- `[skip ci]` - пропустить весь CI процесс
