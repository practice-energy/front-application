      ...
      >
            ...
            <div className="flex flex-wrap gap-2">
              {uploadedFiles.map((file, index) => (
                ...
                >
                  ...
                  <button onClick={() => removeFile(index)} className="text-gray-500 transition-colors duration-200">
                    <X className="w-3 h-3" />
                  </button>
                </div>
              ))}
        ...
        <div className="px-4 py-3 bg-white dark:bg-gray-900 transition-all duration-300 ease-in-out">
          <div className="flex flex-row items-end gap-3 transition-all duration-300 ease-in-out">
                ...
                <form onSubmit={handleSubmit} className="w-full transition-all duration-300 ease-in-out">
                  <div className="flex items-center gap-2.5 transition-all duration-300 ease-in-out">
                    <div className="flex-shrink-0 w-6 h-6 flex items-center justify-center mb-1.5 transition-all duration-300 ease-in-out">
                      <IconAlura width={20} height={20} className="w-4 h-4 transition-all duration-300 ease-in-out" />
                    </div>
                    <div className="flex-1 min-w-0 transition-all duration-300 ease-in-out">
                      <textarea
                        ...
                        className="w-full border-0 bg-transparent text-base placeholder:text-gray-400 focus:outline-none focus:ring-0 resize-none overflow-hidden leading-6 transition-all duration-300 ease-in-out line-clamp-1 leading-relaxed"
                        rows={1}
                        inputmode="text"
                        style={{
                          minHeight: "24px",
                  ...
                  <div className="mobile-search-actions flex items-center justify-between transition-all duration-300 ease-in-out">
                    ...
                    <div
                      className={`transition-all duration-300 ease-in-out ${
                        mode === "input" && isActuallyFocused ? "opacity-100 scale-100" : "opacity-0 scale-90"
                      }`}
                    >
                      <Button
                        type="submit"
            ...
            >
              {mode === "accept" ? (
                <div className="flex items-center justify-center h-14 w-14 mt-0.5">
                  <input
                    type="checkbox"
                    checked={isAccepted}
                    onChange={handleCheckboxToggle}
                    className="h-8 w-8 rounded-sm border-2 border-violet-600 text-violet-600 focus:ring-violet-500 focus:ring-2 cursor-pointer"
                  />
                </div>
              ) : mode === "continue" ? (
                <Button
                  onClick={handleContinue}
                  className="h-14 px-6 mt-0.5 rounded-sm bg-violet-600 hover:bg-violet-700 text-white font-medium"
                >
                  Продолжить
                </Button>
                ...
                <Button
                  ...
                  disabled={!canSubmit}
                  onMouseDown={(e) => e.preventDefault()}
                  className={`h-14 w-14 mt-0.5 p-0 border-none transition-all duration-300 ease-in-out ${
                    canSubmit
                      ? "bg-violet-600 hover:bg-violet-700 text-white"
                      : "bg-violet-200 dark:bg-violet-700 text-white dark:text-gray-500 cursor-not-allowed"
                  }`}
