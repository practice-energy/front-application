...
export function BookingSection({ selectedDate, bookingSlots }: BookingSectionProps) {
  ...
  // Get all time slots that have at least one booking in any of the 3 days
  const getVisibleTimeSlots = () => {
    const slotsWithBookings = new Set<string>()

    days.forEach(day => {
      bookingSlots.forEach(booking => {
        if (booking.date && isSameDay(booking.date, day)) {
          const time = format(booking.date, "HH:mm")
          slotsWithBookings.add(time)
        }
      })
    })

    return Array.from(slotsWithBookings).sort()
  }
  ...
  const getBookingsForDayAndTime = (day: Date, timeSlot: string) => {
    return bookingSlots.filter(booking => {
      ...
      return (
          ...
          format(booking.date, "HH:mm") === timeSlot
      )
    })
  }
              ...
              <div className="grid grid-cols-4 mb-4">
                ...
                {days.map((day, index) => (
                    <div key={index} className="text-center">
                      <div className="text-sm font-medium mx-auto text-neutral-900">
                        {formatDate(day, index === 0)}
                      </div>
                    </div>
                ))}
              </div>
                ...
                <div>
                  {visibleTimeSlots.map((timeSlot) => (
                      <div key={timeSlot} className="grid grid-cols-4 items-center border-b border-l border-gray-200">
                        {/* Time label */}
                        <div className="text-xs text-gray-500 w-[50px] text-right mx-auto">
                          {timeSlot}
                        </div>
                        ...
                        {days.map((day, dayIndex) => {
                          ...
                          return (
                              ...
                              >
                                {hasBookings ? dayBookings.map((booking, bookingIndex) => (
                                    <div
                                        key={bookingIndex}
                                ...
                                )) : <div style={{ height: `${SLOT_HEIGHT}px` }}></div>}
                              </div>
                          )
                        })}
                      </div>
